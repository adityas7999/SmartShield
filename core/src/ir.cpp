...
// The parser-only AST has no reliable referencedDeclaration or stateVariable
// flag. Resolve direct declarations lexically. Never infer storage from a
// variable name alone.
class Builder {
 public:
  ...
  Program build(const json& output) {
    ...
    if (!output.contains("sources") || !output.at("sources").contains(file_))
      throw std::runtime_error(
          "Requested source is missing from compiler output");
    const auto& entry = output.at("sources").at(file_);
    if (!entry.contains("ast") ||
        entry.at("ast").value("nodeType", "") != "SourceUnit")
      throw std::runtime_error("Requested source has no SourceUnit AST");
    program_.typed = entry.at("ast").contains("exportedSymbols");
    for (const auto& node : entry.at("ast").at("nodes")) {
      if (node.value("nodeType", "") == "ContractDefinition")
        ...
        note(no_id, location(node),
             "Source-level " + node.value("nodeType", "unknown") +
                 " is unsupported");
    }
 ...
 private:
  ...
  std::unordered_map<long long, Binding> declaration_bindings_;
  std::unordered_map<IrId, VariableKind> variable_kinds_;
  Program program_;
  IrId next_id_{1};
  ...
  SourceLocation location(const json& node) const {
    ...
    const auto second =
        first == std::string::npos ? first : src.find(':', first + 1);
    if (second == std::string::npos) return loc;
    const auto offset =
        std::from_chars(src.data(), src.data() + first, loc.offset);
    const auto length = std::from_chars(src.data() + first + 1,
                                        src.data() + second, loc.length);
    if (offset.ec != std::errc{} || offset.ptr != src.data() + first ||
        length.ec != std::errc{} || length.ptr != src.data() + second ||
        loc.offset > source_.size() || loc.length > source_.size() - loc.offset)
      return loc;
    const auto line =
        std::upper_bound(line_starts_.begin(), line_starts_.end(), loc.offset);
  ...
  Variable variable(const json& node, VariableKind kind) {
    ...
    if (node.contains("id"))
      declaration_bindings_[node["id"].get<long long>()] = {v.id, kind};
    variable_kinds_[v.id] = kind;
  ...
  Expression expression(const json& node) {
    ...
    if (node.contains("typeDescriptions"))
      e.type_identifier = node["typeDescriptions"].value("typeIdentifier", "");
    e.conversion = node.value("kind", "") == "typeConversion";
    ...
    auto child = [&](const char* key) {
      e.children.push_back(expression(node.at(key)));
    };
    if (type == "Identifier") {
      ...
      if (program_.typed) {
        const auto ref = node.value("referencedDeclaration", 0LL);
        if (declaration_bindings_.contains(ref))
          binding = declaration_bindings_.at(ref);
      } else
        binding = lookup(e.name);
      if (binding) {
      ...
      if (e.op == "&&" || e.op == "||")
        note(e.id, e.location,
             "Short-circuit operands are preserved; conditional effect paths "
             "need expression-level CFG support");
    } else if (type == "UnaryOperation") {
      e.kind = ExpressionKind::unary;
      child("subExpression");
      ...
      for (const auto& item : node.at("components"))
        e.children.push_back(expression(item));
    ...
    if (!e.location.available)
      note(e.id, e.location, "Expression source range is unavailable");
    return e;
  }
  ...
  void collect_call(const Expression& e, Statement& s) {
    ...
    Call call;
    call.id = next_id_++;
    call.expression = e.id;
    call.statement = s.id;
    call.function = function_id_;
    call.location = e.location;
    call.resolution = Resolution::syntax_only;
    if (callee->kind == ExpressionKind::call_options) {
      for (std::size_t i = 0;
           i < callee->names.size() && i + 1 < callee->children.size(); ++i)
        if (callee->names[i] == "value")
          call.value = callee->children[i + 1].id;
      callee = &callee->children.at(0);
    }
    call.name = callee->name;
    call.target = callee->id;
    for (std::size_t i = 1; i < e.children.size(); ++i)
      call.arguments.push_back(e.children[i].id);
    if (e.conversion || callee->kind == ExpressionKind::type)
      call.kind = CallKind::conversion;
      ...
      if (call.name == "call")
        ...
        call.kind = CallKind::transfer;
      else if (call.name == "send")
        call.kind = CallKind::send;
      else if (call.name == "delegatecall")
        call.kind = CallKind::delegatecall;
      else if (call.name == "staticcall")
        call.kind = CallKind::staticcall;
      else
        call.kind = CallKind::external_member;
      if ((call.kind == CallKind::transfer || call.kind == CallKind::send) &&
          !call.arguments.empty())
        call.value = call.arguments.front();
      note(call.id, call.location,
           "Member call classification is syntactic; receiver type and target "
           "are unresolved");
    } else if (callee->kind == ExpressionKind::identifier &&
               !lookup(call.name) && !function_names_.contains(call.name) &&
               (call.name == "require" || call.name == "assert" ||
                call.name == "revert" || call.name == "selfdestruct")) {
      call.kind = CallKind::builtin;
    } else {
      call.kind = function_names_.contains(call.name) && !lookup(call.name)
                      ? CallKind::internal
                      : CallKind::unknown;
      call.resolution = Resolution::unresolved;
      note(call.id, call.location,
           "Callee resolution and interprocedural effects are not implemented");
    }
    if (program_.typed && !e.conversion) {
      const auto& t = callee->type_identifier;
      if (t.find("t_function_barecall_") == 0)
        call.kind = CallKind::low_level;
      else if (t.find("t_function_baredelegatecall_") == 0)
        call.kind = CallKind::delegatecall;
      else if (t.find("t_function_barestaticcall_") == 0)
        call.kind = CallKind::staticcall;
      else if (t.find("t_function_transfer_") == 0)
        call.kind = CallKind::transfer;
      else if (t.find("t_function_send_") == 0)
        call.kind = CallKind::send;
      else if (t.find("t_function_external_") == 0)
        call.kind = CallKind::external_member;
      else if (t.find("t_function_event_") == 0)
        call.kind = CallKind::builtin;
      else if (call.kind != CallKind::builtin &&
               call.kind != CallKind::conversion)
        call.kind = CallKind::internal;
      call.resolution = Resolution::resolved;
    }
    s.calls.push_back(std::move(call));
  }

  // One access for the full path (balances[key]), not another for its base
  // balances.
  void collect_access(const Expression& e, Statement& s, AccessKind action,
                      bool inspect_path = true) {
    const Expression* base = &e;
    std::vector<IrId> keys;
    while ((base->kind == ExpressionKind::index ||
            base->kind == ExpressionKind::member) &&
           !base->children.empty()) {
    ...
    if (!variable_kinds_.contains(base->variable) ||
        variable_kinds_.at(base->variable) != VariableKind::state)
      return;
    std::reverse(keys.begin(), keys.end());
    s.state_accesses.push_back({next_id_++, base->variable, e.id, s.id,
                                function_id_, action, keys, e.location});
  }
  ...
  void collect_facts(const Expression& e, Statement& s) {
    if (e.kind == ExpressionKind::assignment) {
      ...
      if (lhs.kind == ExpressionKind::tuple) {
        note(e.id, e.location,
             "Tuple assignment storage effects are unsupported");
      } else {
    ...
    if (e.kind == ExpressionKind::unary &&
        (e.op == "++" || e.op == "--" || e.op == "delete")) {
      if (e.op != "delete")
        collect_access(e.children.at(0), s, AccessKind::read);
      collect_access(e.children.at(0), s, AccessKind::write, e.op == "delete");
      return;
    }
    if (e.kind == ExpressionKind::identifier ||
        e.kind == ExpressionKind::index || e.kind == ExpressionKind::member) {
  ...
  Statement statement(const json& node, IrId parent) {
    ...
    s.id = next_id_++;
    s.parent = parent;
    s.function = function_id_;
    s.location = location(node);
    const auto type = node.value("nodeType", "");
    ...
    if (type == "Block" || type == "UncheckedBlock") {
      ...
      for (const auto& item : node.at("statements"))
        s.statements.push_back(statement(item, s.id));
      scopes_.pop_back();
      ...
      scopes_.emplace_back();
      s.then_body.push_back(statement(node.at("trueBody"), s.id));
      scopes_.pop_back();
      if (node.contains("falseBody") && node.at("falseBody").is_object()) {
        scopes_.emplace_back();
        s.else_body.push_back(statement(node.at("falseBody"), s.id));
        scopes_.pop_back();
      }
    } else if (type == "ExpressionStatement") {
      s.kind = StatementKind::expression;
      add_expression("expression");
      const auto& e = s.expressions.at(0);
      if (e.kind == ExpressionKind::call && !s.calls.empty() &&
          s.calls.front().kind == CallKind::builtin) {
        const auto& name = s.calls.front().name;
        if ((name == "require" || name == "assert") && e.children.size() > 1) {
          s.kind = name == "require" ? StatementKind::require_guard
                                     : StatementKind::assert_guard;
      ...
      for (const auto& decl : node.at("declarations"))
        if (decl.is_object())
          s.declarations.push_back(variable(decl, VariableKind::local));
        else
          s.declarations.push_back(
              variable(json::object(), VariableKind::local));
    } else if (type == "Return") {
      s.kind = StatementKind::return_statement;
      add_expression("expression");
    } else if (type == "RevertStatement") {
      s.kind = StatementKind::revert_statement;
      add_expression("errorCall");
    } else if (type == "EmitStatement") {
      s.kind = StatementKind::emit;
      add_expression("eventCall");
    } else {
      note(s.id, s.location,
           "Unsupported statement: " + type +
               "; body/effects were not analyzed");
    }
    if (s.calls.size() + s.state_accesses.size() > 1) {
      s.evaluation_order_known = false;
      note(s.id, s.location,
           "Subexpression execution order is unresolved; do not order effects "
           "by vector position");
    }
    return s;
  }
  ...
  Contract contract(const json& node) {
    ...
    c.kind = node.value("contractKind", "contract");
    c.location = location(node);
    scopes_.clear();
    scopes_.emplace_back();
    function_names_.clear();
    c.inherited =
        node.contains("baseContracts") && !node.at("baseContracts").empty();
    if (c.inherited)
      note(c.id, c.location,
           "Inherited declarations and behavior are unresolved");
    for (const auto& item : node.at("nodes")) {
      ...
      if (type == "VariableDeclaration") {
        ...
        if (item.contains("value") && !item.at("value").is_null())
          note(c.variables.back().id, location(item),
               "State initializer effects are not analyzed");
      } else if (type == "FunctionDefinition")
        function_names_[item.value("name", "")] = true;
      else
        note(c.id, location(item),
             "Contract member " + type + " is not expanded");
    }
    for (const auto& item : node.at("nodes")) {
      ...
      f.visibility = item.value("visibility", "");
      f.mutability = item.value("stateMutability", "");
      f.location = location(item);
      scopes_.emplace_back();
      if (item.contains("parameters"))
        for (const auto& p : item.at("parameters").at("parameters"))
          f.parameters.push_back(variable(p, VariableKind::parameter));
      if (item.contains("returnParameters"))
        for (const auto& p : item.at("returnParameters").at("parameters"))
          f.returns.push_back(variable(p, VariableKind::return_value));
      if (item.contains("modifiers") && !item.at("modifiers").empty()) {
        note(f.id, f.location,
             "Modifier applications are not expanded; function reachability is "
             "unresolved");
        for (const auto& modifier : item.at("modifiers")) {
          ...
          if (modifier.contains("arguments") &&
              modifier.at("arguments").is_array())
      ...
      if (item.contains("body") && item.at("body").is_object()) {
        json body = item.at("body");
        if (program_.typed && !c.inherited) {
          for (auto app = f.modifiers.rbegin(); app != f.modifiers.rend();
               ++app) {
            for (const auto& def : node.at("nodes")) {
              if (def.value("nodeType", "") != "ModifierDefinition" ||
                  def.value("name", "") != app->name ||
                  !app->arguments.empty() ||
                  !def["parameters"]["parameters"].empty())
                continue;
              const auto& prefix = def["body"]["statements"];
              bool simple =
                  !prefix.empty() &&
                  prefix.back().value("nodeType", "") == "PlaceholderStatement";
              for (std::size_t i = 0; i + 1 < prefix.size(); ++i) {
                const auto& st = prefix[i];
                simple =
                    simple &&
                    st.value("nodeType", "") == "ExpressionStatement" &&
                    st.contains("expression") &&
                    st["expression"].value("nodeType", "") == "FunctionCall" &&
                    (st["expression"]["expression"].value("name", "") ==
                         "require" ||
                     st["expression"]["expression"].value("name", "") ==
                         "assert");
              }
              if (simple) {
                json expanded = prefix;
                expanded.back() = body;
                body = {{"nodeType", "Block"},
                        {"src", def["body"]["src"]},
                        {"statements", expanded}};
                app->resolution = Resolution::resolved;
              }
            }
          }
        }
        f.body = statement(body, no_id);
      }
      scopes_.pop_back();
...
Program build_ir(const nlohmann::json& output, const std::string& source,
